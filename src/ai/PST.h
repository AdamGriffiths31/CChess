...
struct Score {
    ...
    constexpr bool operator==(Score s) const { return mg == s.mg && eg == s.eg; }
};
...
constexpr Score PAWN_PST[64] = {
    ...
    S(-35,  13), S( -1,   8), S(-20,   8), S(-23, -11), S(-15,  -1), S( 24,  -2), S( 38,   6), S(-22,  -7),
...
// Returns the PST + material value for a piece on a square, from White's perspective.
// Black squares are mirrored via sq ^ 56.
inline Score pstValue(PieceType pt, Color c, Square sq) {
    int pti = static_cast<int>(pt);
    Square s = (c == Color::White) ? sq : (sq ^ 56);
    Score v = MATERIAL_VALUE[pti] + PST[pti][s];
    return (c == Color::White) ? v : -v;
}
