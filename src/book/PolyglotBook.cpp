...
// ---------------------------------------------------------------------------
// Polyglot Random64 table — the fixed, published 781-entry table from the
// Polyglot spec. All opening books use this exact table for key computation.
// Layout:
//   [0..767]   RandomPiece     (12 piece kinds × 64 squares)
//   [768..771] RandomCastle    (wKS, wQS, bKS, bQS)
//   [772..779] RandomEnPassant (8 files a..h)
//   [780]      RandomTurn      (XOR when White to move)
// ---------------------------------------------------------------------------
// clang-format off
// Canonical Polyglot Random64 table (781 entries).
// Source: https://github.com/niklasf/python-chess (POLYGLOT_RANDOM_ARRAY)
// This is the fixed, authoritative table used by all Polyglot-compatible books.
static const uint64_t Random64[781] = {
    // RandomPiece [0..767]: 12 piece kinds × 64 squares
    // kind order: bP=0,wP=1,bN=2,wN=3,bB=4,wB=5,bR=6,wR=7,bQ=8,wQ=9,bK=10,wK=11
    ...
    // RandomTurn [780]: XOR when White to move
...
static int polyglotPieceKind(PieceType pt, Color c) {
    // base: Pawn=0, Knight=2, Bishop=4, Rook=6, Queen=8, King=10
    // offset: Black=0, White=1
    int base = static_cast<int>(pt) * 2;
    int colorOffset = (c == Color::White) ? 1 : 0;
...
uint64_t computePolyglotKey(const Position& pos) {
    ...
    // 1. Piece contributions
    for (Square sq = 0; sq < 64; ++sq) {
        const Piece& p = pos.pieceAt(sq);
        if (p.type() == PieceType::None)
            continue;
        int kind = polyglotPieceKind(p.type(), p.color());
        // Polyglot square index: 8 * rank + file
        int rank = getRank(sq);
        int file = getFile(sq);
        int pgSq = 8 * rank + file;
        key ^= Random64[64 * kind + pgSq];
    }
    ...
    // 3. En passant — only XOR if an adjacent enemy pawn can actually capture.
    //    This differs from FEN semantics which always records the ep square.
    Square ep = pos.enPassantSquare();
    if (ep != SQUARE_NONE) {
        File epFile = getFile(ep);
        Rank epRank = getRank(ep);
        Color stm = pos.sideToMove();
        // The pawn that just double-pushed is on the ep square's rank from
        // the perspective of the capturing side. For White capturing: ep is
        // on rank 5 (index 5), enemy pawn is on rank 4. For Black: ep rank 2,
        // enemy pawn rank 3.
        Rank pawnRank =
            (stm == Color::White) ? static_cast<Rank>(epRank - 1) : static_cast<Rank>(epRank + 1);
        bool adjacent = false;
        if (epFile > 0) {
            Square left = makeSquare(static_cast<File>(epFile - 1), pawnRank);
            const Piece& p = pos.pieceAt(left);
            if (p.type() == PieceType::Pawn && p.color() == stm)
                adjacent = true;
        }
        if (!adjacent && epFile < 7) {
            Square right = makeSquare(static_cast<File>(epFile + 1), pawnRank);
            const Piece& p = pos.pieceAt(right);
            if (p.type() == PieceType::Pawn && p.color() == stm)
                adjacent = true;
        }
        if (adjacent)
    ...
    // 4. Side to move — XOR only for White
...
// ---------------------------------------------------------------------------
// Big-endian read helpers (Polyglot files are big-endian)
// ---------------------------------------------------------------------------
...
std::optional<Move> decodePolyglotMove(uint16_t pgMove, const Board& board) {
    ...
    //   bits [8:6]   = from file
    //   bits [11:9]  = from rank
    //   bits [14:12] = promotion piece (0=none,1=N,2=B,3=R,4=Q)
    int tf = (pgMove >> 0) & 7;
    int tr = (pgMove >> 3) & 7;
    int ff = (pgMove >> 6) & 7;
    int fr = (pgMove >> 9) & 7;
    ...
    Square from = makeSquare(static_cast<File>(ff), static_cast<Rank>(fr));
    Square to = makeSquare(static_cast<File>(tf), static_cast<Rank>(tr));

    // Remap Polyglot castling encoding (king-to-rook-corner)
    // to the engine's encoding (king-to-destination square).
    //   White KS: e1→h1 → e1→g1
    //   White QS: e1→a1 → e1→c1
    //   Black KS: e8→h8 → e8→g8
    //   Black QS: e8→a8 → e8→c8
    constexpr Square E1 = makeSquare(FILE_E, RANK_1);
    constexpr Square E8 = makeSquare(FILE_E, RANK_8);
    if (from == E1) {
        if (to == makeSquare(FILE_H, RANK_1))
            to = makeSquare(FILE_G, RANK_1);
        else if (to == makeSquare(FILE_A, RANK_1))
            to = makeSquare(FILE_C, RANK_1);
    } else if (from == E8) {
        if (to == makeSquare(FILE_H, RANK_8))
            to = makeSquare(FILE_G, RANK_8);
        else if (to == makeSquare(FILE_A, RANK_8))
            to = makeSquare(FILE_C, RANK_8);
    }

    // Map promotion piece index to PieceType
    static const PieceType promoTable[] = {PieceType::None, PieceType::Knight, PieceType::Bishop,
                                           PieceType::Rook, PieceType::Queen};
    PieceType pt = (promo >= 1 && promo <= 4) ? promoTable[promo] : PieceType::None;

    // Validate legality (guards against hash collisions / stale entries)
    return board.findLegalMove(from, to, pt);
}

// ---------------------------------------------------------------------------
// PolyglotBook implementation
// ---------------------------------------------------------------------------
...
bool PolyglotBook::load(const std::string& path) {
    ...
    while (f.read(reinterpret_cast<char*>(buf), kEntrySize)) {
        BookEntry e;
        e.key = readU64Be(buf + 0);
        e.move = readU16Be(buf + 8);
        e.weight = readU16Be(buf + 10);
        e.learn = readU32Be(buf + 12);
    ...
    // Polyglot files are sorted by key, but sort defensively
...
std::optional<uint16_t> PolyglotBook::probe(uint64_t key) const {
    if (entries_.empty())
        return std::nullopt;
    ...
    // Collect all entries for this position (weight > 0 only)
    uint32_t total = 0;
    auto start = it;
    ...
    if (total == 0) {
        // All entries have weight 0 — Polyglot convention: "never play".
        // Fall through to search rather than playing an undesirable move.
        return std::nullopt;
    }
    ...
    uint32_t cum = 0;
    for (auto jt = start; jt != entries_.end() && jt->key == key; ++jt) {
        if (jt->weight == 0)
            continue;
        cum += jt->weight;
        if (r < cum)
    ...
    return start->move;  // fallback (should not be reached)
}
